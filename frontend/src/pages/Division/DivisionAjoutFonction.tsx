...
export default function DivisionAjoutFonction({
  visible,
  onHide,
  division,
}: any) {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      await createFonction({ libelle, division_id: division.id });
      setLibelle("");
      toast.current?.show({
        severity: "success",
        summary: "Succès",
    ...
    >
      <div className="pt-4 space-y-5">
        <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100">
          ...
          <p className="text-emerald-900 font-bold">{division?.libelle}</p>
        </div>
        <div>
