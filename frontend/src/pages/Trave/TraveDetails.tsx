...
import { Box, Trave } from "../../interfaces";
import {
  BoxIcon,
  MapPin,
  Hash,
...
interface TraveDetailsProps {
  visible: boolean;
  onHide: () => void;
  trave: Trave | null;
}

const TraveDetails = ({ visible, onHide, trave }: TraveDetailsProps) => {
    ...
    <Dialog
      header={
        <div className="flex items-center gap-3 text-slate-800">
          ...
          <span className="font-bold">Détails du traver</span>
        </div>
      }
      visible={visible}
      onHide={onHide}
          ...
          <div className="bg-gradient-to-br from-green-600 to-emerald-700 text-white p-6 rounded-2xl shadow-md">
            ...
            <div className="flex flex-wrap gap-4 text-sm opacity-90">
              <div className="flex items-center gap-1">
                <Hash size={16} /> <span>Code: {trave.code}</span>
              </div>
              <div className="flex items-center gap-1">
                <MapPin size={16} />{" "}
                <span>Salle: {trave.rayon?.code || "Non définie"}</span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            {loading ? (
              ...
              <div className="text-center p-10 border-2 border-dashed border-slate-200 rounded-2xl">
                ...
                <p className="text-slate-400">Aucun box sur ce trave</p>
              </div>
            )}
          </div>
        </div>
