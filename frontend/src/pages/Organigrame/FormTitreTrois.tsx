...
// ✅ NOUVEL IMPORT
import { deleteEntiteeTroisTitre } from "../../api/entiteeTrois";
...
export default function FormTitreTrois({
  visible,
  onHide,
  onSubmit,
  refresh,
  ...
  const [entiteeDeuxId, setEntiteeDeuxId] = useState(null);
  ...
  const handleSubmit = async () => {
    if (!libelle.trim() || !entiteeDeuxId) return;

    setLoading(true);
    try {
      await createEntiteeTrois({
        libelle,
        entitee_deux_id: entiteeDeuxId,
      });
      onSubmit({ libelle, entiteeDeuxId });
      refresh();
      onHide();
    } catch (error) {
  ...
  const handleUpdateTitre = async () => {
    ...
    setLoading(true);
    try {
      await updateEntiteeTroisTitre(newTitre);
      if (onTitreUpdate) {
  ...
  const handleDeleteTitre = async () => {
    ...
    confirmDialog({
      ...
      accept: async () => {
        try {
          const result = await deleteEntiteeTroisTitre();
    ...
    <>
      ...
      >
        <div className="relative">
          {/* HEADER */}
          <div className="bg-gradient-to-br from-purple-600 to-purple-900 p-6 text-white">
            <button
              onClick={onHide}
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                  {titles?.entitee2 || "Entité parente"} (Niveau 2)
                </label>
                <Dropdown
                  value={entiteeDeuxId}
                  onChange={(e) => setEntiteeDeuxId(e.value)}
                  options={entiteeDeux}
                  optionLabel="libelle"
                  optionValue="id"
                  placeholder={`Sélectionner un(e) ${titles?.entitee2 || "parent"}`}
                  className="w-full"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <InputText
                  ...
                  placeholder={`Ex: ${currentTitre || "Service"} Informatique...`}
                  className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500"
                />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
              ...
              <Button
                ...
                loading={loading}
                disabled={!libelle.trim() || !entiteeDeuxId || deleteLoading}
                className="bg-purple-600 text-white px-8 py-3 rounded-xl shadow-lg hover:bg-purple-700 transition-all"
              />
            </div>
          </div>
        </div>
